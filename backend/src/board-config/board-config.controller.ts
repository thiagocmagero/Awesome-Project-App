...
export class BoardConfigController {
  constructor(private readonly service: BoardConfigService) {}
  ...
  @Put('global')
  upsertGlobal(@Body() dto: UpsertBoardConfigDto, @CurrentUser() user: JwtPayload) {
    assertAdmin(user);
  ...
  @Put('user')
  upsertUser(@Body() dto: UpsertBoardConfigDto, @CurrentUser() user: JwtPayload) {
  ...
  @RequireProjectPermission(ProjectAction.BOARD_CONFIG)
  upsertProject(
    @Param('projectId', ParseUUIDPipe) projectId: string,
    @Body() dto: UpsertBoardConfigDto,
    @CurrentUser() user: JwtPayload,
  ) {
