// Demo visual do componente AwesomeKanban (Aw-Kanban) — fase de teste UI,
// sem qualquer integração com backend, BD ou serviços.
//
// Usa modo UNCONTROLLED (defaultCards / defaultColumns / defaultRows):
// o componente gere o seu próprio estado internamente. Ao usar modo
// controlled (cards / columns / rows) o setCards interno é NOOP, o que
// causa snap-back visual quando um card é reordenado na mesma coluna
// (o widget actualiza o estado interno mas o prop externo ainda não
// chegou, criando um flash com a ordem antiga antes do re-render React).
...
// ─── Demo data (verbatim from C:\dev\Aw-Kanban\src\demo\App.tsx) ─────────────
...
// ─── Component ───────────────────────────────────────────────────────────────
export function KanbanDemoView() {
  ...
  // Toggle swimlanes via the API — cannot use the `rows` controlled prop in
  // uncontrolled mode because it conflicts with the component's internal state.
  useEffect(() => {
    apiRef.current?.parse({ rows: showSwimlanes ? initialRows : [] });
  }, [showSwimlanes]);
