/**
 * Helpers para extrair contexto de erros de upload e formatar mensagens i18n.
 *
 * O backend `AppException` serializa o body com `{ error_code, ...context }`.
 * O `useFiles` propaga o body completo via `throw new Error(JSON.stringify(...))`.
 * Este módulo dá ferramentas para parse + render no UI.
 */
