// Port adaptado de `frontend/src/features/files/useFiles.ts` (regra 4).
// Adaptações face ao legacy:
//   - `apiFetch` do frontend2 (cookies HttpOnly automáticos, sem header Authorization).
//   - `getApiBase()` igual ao legacy (`/api/v1`).
//
// Pattern: optimistic em delete; pessimistic em upload/replace/rename.
...
export function useFiles(opts: UseFilesOpts): UseFilesResult {
  ...
  const replace = useCallback(
    async (filePublicId: string, file: File): Promise<AppFile | null> => {
      ...
      setFiles((curr) => curr.map((f) => (f.publicId === filePublicId ? updated : f)));
      return updated;
    },
    [],
  );
